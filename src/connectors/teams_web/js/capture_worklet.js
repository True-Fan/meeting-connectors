/*
 * mc-teams-capture — the meeting's audio, out of Teams' own playout graph and into the
 * avatar's input format.
 *
 * Runs on the Web Audio rendering thread inside an AudioContext that `inject.js` constructs
 * with `{ sampleRate: 16000 }`. That single constructor option is what makes this file
 * trivial: the browser's own graph resamples whatever Teams is rendering — 48 kHz in the
 * usual case — down to 16 kHz in native code before this processor sees a sample, so there
 * is no resampler here, no filter design and no aliasing to reason about.
 *
 ...
 * WHY THIS DOWNMIXES
 * ------------------
 * The Google Meet capture node is fed one `MediaStreamAudioSourceNode` per remote track,
 * each of which the graph presents as mono. This one is fed whatever the page connected to
 * its destination, which for a conference upmixed to stereo is two channels carrying the
 * same signal. Reading `input[0]` alone would take the left channel only — which is not
 * silence, so it would never be noticed, and would quietly halve the level of anything
 * panned right.
 *
 ...
 * Emitting partial frames instead would push that arithmetic onto the Python boundary, where
 * `AudioFrame` would reject it.
 */

class McTeamsCaptureProcessor extends AudioWorkletProcessor {
  constructor(options) {
    super();
  ...
  process(inputs) {
    ...
    // No connected source yet, or Teams has torn its graph down. Silence is the truthful
...
registerProcessor('mc-teams-capture', McTeamsCaptureProcessor);
