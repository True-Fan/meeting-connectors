...
class TeamsWebSelectors:
    ...
    camera_on_button: tuple[str, ...] = (
        # **Unverified against a live meeting — best-effort, same status every selector list
        # in this file started at.** Teams labels this control by what a click *does*, the
        # same convention ``unmute_button`` relies on, so "Turn camera on" showing means the
        # camera is off now. Never match "Turn camera off" here: that would turn it back off.
        "button[data-tid='toggle-video'][aria-label*='turn camera on' i]",
        "button[aria-label*='turn camera on' i]",
        "[role='button'][aria-label*='turn camera on' i]",
        "button[title*='turn camera on' i]",
        "button:has-text('Turn camera on')",
    )
...
class TeamsWebJoiner:
    ...
    async def ensure_camera_on(self, attempts: int = 8) -> bool:
        ...
        **Public, unlike ``_ensure_unmuted``**, because it is not part of the join sequence
        ...
        the page is confirmed connected (see ``TeamsWebSession.start``).

        Same shape as ``_ensure_unmuted`` for the same reason: a single click is not enough,
        because Teams may not have rendered the control yet on the first attempt, and success
        is defined by the control disappearing rather than by the click itself succeeding —
        "Turn camera on" absent means the camera is on, since Teams names the button after
        ...
        logger.warning(
            "teams_web.camera_still_off",
            note="the avatar is in the meeting but its camera is off, so nothing it "
            "publishes will be seen; the turn-camera-on control did not clear — this "
