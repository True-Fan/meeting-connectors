...
Two responsibilities, deliberately separate: deciding whether a message was addressed to the
avatar at all, and handing the survivors to the router as they arrive. Parsing is not one of
them — the page has already reduced a chat item to a sender and a line of text, and
``meeting/observer.py`` has already turned that into a domain ``ChatMessage``.

**Why the ``@`` is required by default.** A meeting's chat is a conversation between people:
participants swap links, greet each other, and answer among themselves. An avatar that speaks
up after every line is interrupting a room that was not talking to it. Requiring the tag makes
answering opt-in per message, which is how a person in the room behaves.

**Why the page cannot be trusted to decide it.** What deserves an answer is policy, and policy
lives in Python beside the settings that govern it. The page reports what was typed; this
...
def _mention_pattern(name: str) -> re.Pattern[str] | None:
    ...
    **The ``@`` is required.** Teams' compose box resolves an ``@`` mention into a rich entity,
    and what the chat panel renders for it is still the display name preceded by ``@`` — so the
    tag survives into the DOM this reads, and it is the difference between talking *to* the
    avatar and talking *about* it. "did the AI avatar join?" is a question for the room; "@AI
    Avatar are you there?" is a question for the avatar.
...
def strip_mention(text: str, names: Sequence[str]) -> str | None:
    ...
    for name in names:
        ...
        # Only the gap the removal left, not every run: line structure is left alone because a
        # pasted question can carry meaningful newlines.
        remainder = re.sub(r"[ \t]{2,}", " ", remainder)
        return remainder or text
    return None


class TeamsChatSource:
    """``ChatSource`` fed by messages the page read out of the chat panel.
    ...
    def observe_self_name(self, name: str | None) -> None:
        ...
        logger.info("teams_chat.mention_name", name=cleaned, total=len(self._mention_names))

    def observe_participants(self, present: tuple[str, ...]) -> None:
        """Learn who else is in the meeting, for elimination. Never raises.

        Reachable whenever the page reads a chat item whose author element it could not
        resolve — which a virtualised list makes more likely here than on a connector receiving
        chat as data. A nameless question answered with "I don't know your name" to somebody the
        roster knew all along is an expensive failure where the fix is four lines.
        """
        try:
            names: list[str] = []
            ...
            logger.warning("teams_chat.participants_failed", error=str(exc))
    ...
    def offer(self, message: ChatMessage) -> bool:
        ...
        Called from the page read loop, which also carries the avatar's voice into the page —
        so this must not be able to stall it or throw into it. Both properties are why it is a
        plain method returning a bool rather than a coroutine that might await.
        """
        try:
            return self._offer(message)
        except Exception as exc:  # pragma: no cover - defensive
            logger.warning("teams_chat.offer_failed", error=str(exc))
            return False
    ...
    def _offer(self, message: ChatMessage) -> bool:
        ...
        # **The avatar's own messages, recognised by name.** The chat panel labels a message
        # with the sender's display name and nothing that says "this is you", so ``is_self``
        # arrives False on the avatar's own line. Left as it was, an avatar that ever posts to
        # the chat would answer itself — the text-channel version of an echo loop.
        if not message.is_self and self._is_self(message.sender):
            message = replace(message, is_self=True)

        attributed_by = "page" if message.sender else "none"
    ...
    def _addressed(self, message: ChatMessage) -> ChatMessage | None:
        ...
        if not self._mention_names:
            ...
            if not self._warned_nameless:
                ...
                logger.warning(
                    "teams_chat.no_mention_name",
                    note="chat requires an @mention but no name to match is known yet; set "
                    "MC_TEAMS_WEB__CHAT_MENTION_NAMES, or "
                    "MC_TEAMS_WEB__CHAT_REQUIRE_MENTION=false to answer every message",
                )
            return None
    ...
    async def messages(self) -> AsyncIterator[ChatMessage]:
        """Yield chat messages as the page reports them."""
        while True:
    ...
    def health(self) -> ComponentHealth:
        ...
        where nobody has typed anything, so claiming otherwise would be invention. The counts
        are in the detail, which is what an operator checking whether the feature is live
        actually needs.
        """
