...
COMPONENT_NAME = "zoom_web_ingest"


class PageAudioSource:
    """Live Zoom conference audio, tapped in the page and delivered over the page channel."""
    ...
    def __init__(
        ...
        *,
        ...
        self._queue: BoundedFrameQueue[AudioFrame] = BoundedFrameQueue(
            name="zoom_web_ingest",
            maxsize=max(queue_size, 1),
            # Realtime: a stale frame of a conversation has no value, and the alternative
            # is refusing new audio while holding audio nobody will hear in time.
    ...
    async def start(self) -> None:
        ...
        There is nothing to *open*: the page channel is already bound and the page is
        already tapping. Registering the handler is the whole of it, and doing it here
        rather than at construction means a session that never starts never receives a
        frame it would have to discard.
        """
        if self._started:
            return
        self._started = True
    ...
    async def stop(self) -> None:
        ...
        **The page server is not stopped here**, deliberately: it is shared with the
        publisher, and stopping it from the ingest leg would take the avatar's voice down as
        a side effect of ingest shutting up. ``ZoomWebSession`` stops it once, for both.
        """
    ...
    def _on_pcm(self, pcm: bytes) -> None:
        ...
        **Synchronous, non-blocking and total**, which is a hard requirement rather than a
        ...
        pulling costs counted drops rather than backpressure onto a socket that cannot take
        it.
        ...
        ``capture_worklet.js`` only ever posts complete frames, so this is a skew check
        ...
        frame = AudioFrame(
            ...
            # arbitrary origin that drifts against the monotonic clock the rest of the
            # pipeline is paced on. Mixing the two corrupts the single-clock invariant A/V
            # sync depends on. Same rule as ``google_meet/audio_capture/mapping.py``.
            pts_us=self._clock.now_us(),
            # Asserted rather than converted: the capture ``AudioContext`` is constructed at
            # 16 kHz so Web Audio resamples in native code before the worklet sees a sample.
            # A page running a stale script that built a 48 kHz context would produce a
            # chipmunk voice three services downstream; this is where that is caught.
    ...
    def health(self) -> ComponentHealth:
        ...
        **Not unhealthy while silent**, and the distinction is the whole value of this
        report. A meeting in which nobody has spoken yet produces no frames and is perfectly
        fine. A tap that never found Zoom's audio graph also produces no frames and is
        completely broken. They are indistinguishable from here, so this reports the fact
        (``tapped=0``) and refuses to editorialise — ``page_event`` lines carrying
        ``audioTapped`` are what settle it, and they are logged the moment a source is wired.
        """
        ...
        if not self._started:
            # ``UNKNOWN`` rather than ``UNHEALTHY``: not-yet-started is what that state is
            # for, and calling it unhealthy would make every session look broken for the
            # window between construction and ``start``.
        ...
        if tapped == 0:
            return ComponentHealth(
                ...
                detail=(
                    ...
                    f"meeting or a tap that never attached to Zoom's playout graph — {detail}"
                ),
            )
        return ComponentHealth(
