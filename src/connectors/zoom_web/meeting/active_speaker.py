"""Who is speaking now, and who has spoken, from Zoom's own active-speaker events.

**This is the module where Zoom's advantage over Meet is largest, and it shows up as
what is missing.** The Meet connector assembles attribution from two indirect
observations — per-track audio energy, which says *when* but never *who*, and a
participant tile, which says *who* but only once Meet has drawn it — and most of that
file is the machinery for reconciling them: renaming an open turn when its identity
arrives late, merging two observers reporting one person, adopting an anonymous turn
when a name lands over it, eliminating down to the only other participant in the room.
Every one of those exists because neither signal alone is trustworthy.

Zoom raises ``ACTIVE_SPEAKER_CHANGE`` with the participant on it. There is one signal,
it is authoritative, and it names somebody — so none of that machinery is here, and
adding it would be inventing ambiguity in order to resolve it.

**What Zoom's signal does not give, and how this covers it.** It is a *level*, not a
...
sentences would attribute half the interruptions in a meeting to no one. The merge gap,
because one person talking through a pause is one turn and not two — without it, "who
has been speaking" answers with the same name forty times. And the rule that
``current_speaker`` is a dictionary lookup and nothing more, because the media router
...
participant — and reachable when an event carries only a user id the roster has not
explained yet. The answer has to name somebody, and an honest placeholder beats a
confident guess."""

_MAX_NAME_LEN = 120
_MAX_TURNS = 2_000
"""Ceiling on remembered turns. Past it the *oldest* go: this answers "who is talking and
who just talked", where the newest matter most — the opposite of the attendance ledger's
...
class SpeakerTurn:
    ...
    inferred: bool = False
    """True when the name was reached by elimination — exactly one other person was in
    the meeting — rather than read off Zoom's event. Kept distinct because they are
    different claims, and a caller that cares should not have to guess which it has."""
...
class SpeakerSnapshot:
    ...
    Field-for-field compatible with the Google Meet connector's snapshot, so
    ``GET /sessions/{id}/speakers`` serves a Zoom session through the same duck-typed
    path — see ``AttendanceSnapshot`` for why that compatibility is a design choice
    rather than a coincidence.
    """

    turns: tuple[SpeakerTurn, ...] = field(default_factory=tuple)
    current: tuple[str, ...] = field(default_factory=tuple)
    """Everyone speaking as of now. **At most one on this connector**, because Zoom
    reports a single active speaker rather than a set — a tuple because the shared
    response shape carries one, and because reporting a plural honestly is better than
    a field that lies about its cardinality."""

    self_name: str | None = None
    events: int = 0
    """Speaker changes observed. ``0`` is the difference between "nobody has spoken" and
    "we do not know"."""

    candidates: tuple[str, ...] = field(default_factory=tuple)
    """Everyone present who could be the voice, the avatar excluded. Carried so the brief
    can name the field even before the first speaker change arrives."""
    ...
    def talk_time(self) -> tuple[tuple[str, int], ...]:
        ...
        Summed across turns rather than measured end-to-end, so somebody who spoke twice
        for a minute each reads as two minutes rather than as however long the meeting
        took.
        """
        totals: dict[str, int] = {}
    ...
    def agent_context(self) -> str:
        ...
        *not* known, deliberately: an agent told only that somebody is speaking will answer
        ...
        if not self.events:
            heard = (
                "Nobody has been heard speaking yet — Zoom has not reported anyone taking "
                "the floor."
            )
        ...
        if self.current:
            ...
            if speaker == ANONYMOUS:
                ...
                # the same wording the Meet brief arrived at after an avatar answered "who is
        ...
        # paragraph is built from Zoom's active-speaker events: somebody who has only typed
        # in the chat has never taken the floor and does not appear here. An agent that
        # reads "recent speakers" as "people who have communicated" answers "who was
        # talking to you?" with whoever typed last.
        lines.append(
            "This paragraph counts speech only: somebody who typed in the chat but has "
            "not spoken is not the voice being heard."
        )

        if self.self_name:
            lines.append(
                f'The avatar itself appears as "{self.self_name}" and is never counted as '
                "a speaker."
            )
    ...
    def _unattributed_guard(self) -> str:
        ...
        model to resolve it from whatever else is in the frame — and what is in the frame
        is a chat history with one name in it. The answer *is* one of these people, which
        is real information, and the wrong inference is ruled out explicitly.
        """
        if not self.candidates:
            return (
                "Do not guess who it is, and do not assume it is whoever typed in the "
                "chat — ask if it matters."
            )
        who = (
        ...
        return (
            ...
            "typing in the chat is not the voice being heard — and ask who is speaking if "
            "the answer matters."
        )


class ZoomSpeakerTracker:
    """Turns Zoom's active-speaker events into a current speaker and a turn history."""
    ...
    def observe(self, event: SpeakerEvent) -> bool:
        """Accept one active-speaker change. Never blocks, never raises.
        ...
        try:
            ...
            logger.warning("zoom_speaker.observe_failed", error=str(exc))
            return False
    ...
    def _observe(self, event: SpeakerEvent) -> bool:
        ...
        inferred = False
        if not name:
            # Elimination, and on this connector it is a repair rather than the main route:
            # Zoom names the speaker, so this only covers an event that arrived with an id
            # the roster has not explained yet. Fails closed at two or more others, because
            # naming one of two would be a guess and a confident wrong name is worse than
            # "Someone".
        ...
        if self._is_self(name):
            # The avatar is a participant, so Zoom reports it taking the floor whenever it
            # speaks. Counting that would have the avatar report itself as the speaker for
            # as long as it talks — and, worse, would name it as the person to yield to.
            self._ignored += 1
            return False
        ...
        if open_turn is not None and open_turn.identity == identity:
            # The same person still holding the floor. Zoom repeats the event; treat it as
            # an identity refresh rather than a new turn, so one uninterrupted speech does
            # not read as a dozen.
            self._improve(open_turn, user_id=event.user_id, name=name or None)
            return False

        # The floor has moved. Zoom sends no "stopped", so closing the previous turn is
        # this class's job and this is the only place it happens on a live meeting.
        ...
        self._open = turn
        logger.info(
            "zoom_speaker.started", participant=turn.label, attributed=bool(name)
        )
        return True
    ...
    def observe_participants(self, present: tuple[str, ...]) -> None:
        ...
        What ``_only_other`` eliminates against, and what the brief names as candidates.
        Taken from the ledger rather than tracked here because the ledger already holds it
        and two accounts of who is present is one account too many.
        """
        try:
            others = [
                name
    ...
    def _resume(self, identity: str, *, now_us: int) -> _Turn | None:
        ...
        What makes the history read like a conversation rather than like a waveform: two
        people alternating quickly are two people talking, not forty turns.
        """
    ...
    def _backfill_names(self) -> None:
        """Name turns recorded before anything explained whose id they were.

        Retroactive on purpose: somebody who spoke in the first seconds of the call would
        otherwise stay "an unidentified participant" in the history for the rest of the
        meeting, when we now know exactly who they were.
        """
        only = self._only_other()
    ...
    def ignored(self) -> int:
        """Events seen and not counted — the avatar's own turns."""
    ...
    def current_speaker(self) -> str | None:
        ...
        Read from the media router's inbound leg when somebody barges in, so it is a
        couple of comparisons and nothing else — no I/O, no lock, no allocation worth
        naming.
        """
    ...
    def _current(self, now_us: int) -> tuple[str, ...]:
        ...
        if not self._hold_us:
            return ()
        # Nobody holds the floor this instant. Somebody who stopped a moment ago still
        # counts as the person speaking — speech has gaps at every clause boundary, and an
        # answer that flickers to "nobody" between two words is the wrong answer to the
...
def _identity(*, user_id: int | None, name: str | None) -> str:
    ...
    Folded name first, and that inverts the obvious ordering for the reason the
    attendance ledger's key does: Zoom's ``user_id`` is exact within one stay and minted
    afresh on rejoin, so keying on it would make one person who reconnected two speakers
    with separate talk times. The id is the fallback for somebody Zoom has not named.
    """
    if name:
